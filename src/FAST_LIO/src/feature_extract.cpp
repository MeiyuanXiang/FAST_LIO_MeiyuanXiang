...
/*
  feature_extract主要提取激光点云中的平面点和边缘点，发布/laser_cloud、/laser_cloud_flat、/laser_cloud_sharp。
*/
...
int main(int argc, char **argv)
{
  ...
  // 从launch读取配置参数
  n.param<int>("lidar_type", lidar_type, 0);                    // 雷达类型，包含MID40、HORIZON、VELO16、OUST64
  n.param<double>("blind", blind, 0.5);                         // 距离最小值，用于剔除重复点，当当前点与后一个点距离的平方小于该阈值，则剔除
  n.param<double>("inf_bound", inf_bound, 10);                  // 无穷值阈值，超出该阈值则认为是无穷值
  n.param<int>("N_SCANS", N_SCANS, 1);                          // 扫描线数
  n.param<int>("group_size", group_size, 8);                    // 激光点云特征提取中，用于确实是否分组，点数大于该阈值则分组
  n.param<double>("disA", disA, 0.01);                          // 平面点判断算法中，用于增大点计算过程中的搜索范围，相乘的系数
  n.param<double>("disB", disB, 0.1);                           // 平面点判断算法中，用于增大点计算过程中的搜索范围，相加的稀疏
  n.param<double>("p2l_ratio", p2l_ratio, 400);                 // 用于判断是否为平面点的阈值，如果小于该阈值，则该点是待判断点
  n.param<double>("limit_maxmid", limit_maxmid, 9);             // 检测平面点中的阈值
  n.param<double>("limit_midmin", limit_midmin, 16);            // 检测平面点中的阈值
  n.param<double>("limit_maxmin", limit_maxmin, 3.24);          // 检测平面点中的阈值
  n.param<double>("jump_up_limit", jump_up_limit, 175.0);       // 角度上限
  n.param<double>("jump_down_limit", jump_down_limit, 5.0);     // 角度下限
  n.param<double>("cos160", cos160, 160.0);                     // 160°
  n.param<double>("edgea", edgea, 3);                           // 有跨越的边判断算法中，距离阈值，相乘的系数
  n.param<double>("edgeb", edgeb, 0.05);                        // 有跨越的边判断算法中，距离阈值，相加的稀疏
